...
if __name__ == "__main__":
    ...
    # Test RGB
    # ==== Choose an image
    frames_sample = img_dataset[12]["frames"]
    os.makedirs(out_path, exist_ok=True)
    for t in range(frames_sample.shape[1]):
        frame = frames_sample[:, t]  # shape: [3, 256, 256]
        save_image(frame, os.path.join(f"{out_path}/frame_{t}.jpg"))
    ...
    for t in range(vid_dec_sample.shape[1]):
        frame = vid_dec_sample[:, t]  # shape: [3, 256, 256]
        save_image(frame, os.path.join(f"{out_path}/frame_decod_{t}.jpg"))
